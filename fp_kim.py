...
def farthest_points_sampler(k, path):
    ...
    if do_plot:
        ...
        # fig = plt.figure()
        # ax = fig.gca(projection='3d')
        # ax.scatter(pts[farthest_indices, 0], pts[farthest_indices, 1], pts[farthest_indices, 2])
        # plt.show()
        # plt.savefig('results/farthest_point_sampling_.png')

        # --- To extract farthest_points, you can use this kind of advanced indexing ---
        farthest_points = pts[farthest_indices, :][0]
        print('farthest_points', farthest_points.shape)

        farthest_pts_index = pts[farthest_indices, :][0]
        print('farthest', farthest_points.shape,
              farthest_pts_index.shape,
              numpy.sum(farthest_points - farthest_pts_index))
        assert numpy.allclose(farthest_points, farthest_pts_index)

        return farthest_points, farthest_indices